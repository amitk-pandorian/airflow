...
test.describe("Connections Page - List and Display", () => {
  ...
  test.afterAll(async ({ authenticatedRequest }) => {
    const response = await authenticatedRequest.delete(`/api/v2/connections/list_seed_conn_${timestamp}`);

    if (response.status() !== 404 && !response.ok()) {
      console.warn(`Cleanup failed for list_seed_conn_${timestamp}: ${response.status()}`);
    }
  });
...
test.describe("Connections Page - CRUD Operations", () => {
  ...
  test.afterAll(async ({ authenticatedRequest }) => {
    for (const connId of [existingConnection.connection_id, ...createdConnIds]) {
      ...
      // 404 = already deleted; any other failure is unexpected but non-blocking for test results.
      if (response.status() !== 404 && !response.ok()) {
