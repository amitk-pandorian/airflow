...
export class ConnectionsPage extends BasePage {
  ...
  public async fillConnectionForm(details: Partial<ConnectionDetails>): Promise<void> {
    ...
    if (details.conn_type !== undefined && details.conn_type !== "") {
      // Flexible locator — chakra-react-select may not render role="combobox" on WebKit.
      const selectCombobox = this.connectionForm
        .locator('[role="combobox"], .chakra-react-select input')
        .first();
      ...
      }).toPass({ intervals: [2000, 3000], timeout: 60_000 });
      ...
      try {
        ...
        // Fallback: type directly when dropdown option click fails.
  ...
  public async saveConnection(): Promise<void> {
    ...
    // Wait for the dialog backdrop to fully detach from the DOM after close animation.
    // Without this, webkit may leave a stale backdrop that blocks subsequent dialog interactions.
    await this.page
