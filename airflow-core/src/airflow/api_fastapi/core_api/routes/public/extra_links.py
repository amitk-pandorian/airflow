...
def get_extra_links(
    ...
    all_extra_links: dict[str, LinkResponseValue | None] = {}
    for link_name in sorted(task.extra_links):
        ...
        link_obj = getattr(task, "operator_extra_link_dict", {}).get(link_name) or getattr(
            task, "global_operator_extra_link_dict", {}
        ).get(link_name)
        target = getattr(link_obj, "target", "_blank") if link_obj else "_blank"

        if url is not None:
            all_extra_links[link_name] = LinkResponseValue(url=url, target=target)
        else:
            all_extra_links[link_name] = None
